...
func create(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		...
		if db.Where("username = ? OR email = ?", payload.Username, payload.Email).First(&existingUser).Error == nil {
			w.Header().Set("Content-Type", "application/json")
		...
		user := User{Username: payload.Username, Password: string(hashedPassword), Email: payload.Email}
...
func signup(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		...
		user := User{Username: userDetails.Username, Password: string(hashedPassword), Email: userDetails.Email}
		result := db.Create(&user)
		if result.Error != nil {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusInternalServerError)
...
func main() {
	var err error
	dsn := "root:kl18jda183079@tcp(localhost:3306)/logindata?charset=utf8mb4&parseTime=True&loc=Local"
